...
st.title("AI Stock Advisor - Real-time Insights for AAPL")

# Sections for real-time data and key insights
st.header("Current Market Summary")
metrics_section = st.container()
plot_section = st.container()
log_section = st.container()
insight_section = st.container()

# Real-time log and insights setup
logtxtbox = log_section.empty()
logtxtbox.caption("Market Opened at 09:30:00")
message = insight_section.empty()
insight_log = []
...
rolling_window = pd.DataFrame()
dow_rolling_window = pd.DataFrame()
daily_high, daily_low, buying_momentum, selling_momentum = float('-inf'), float('inf'), 0, 0
...
def process_stock_update():
    ...
    if not data.empty and not dow_data.empty:
        ...
        update = data.iloc[0].to_frame().T
        dow_update = dow_data.iloc[0].to_frame().T
        data, dow_data = data.iloc[1:], dow_data.iloc[1:]

        # Update rolling windows
        rolling_window = pd.concat([rolling_window, update], ignore_index=False)
        dow_rolling_window = pd.concat([dow_rolling_window, dow_update], ignore_index=False)

        # Update daily high, low, and momentum
        update_price = update['Close'].values[0]
        daily_high, daily_low = max(daily_high, update_price), min(daily_low, update_price)
        price_change = update_price - rolling_window['Close'].iloc[-2] if len(rolling_window) >= 2 else 0
        if price_change > 0:
            buying_momentum += price_change
        else:
            selling_momentum += abs(price_change)

        # Maintain a rolling window of the last 5 minutes
        if len(rolling_window) > 5:
            rolling_window, dow_rolling_window = rolling_window.iloc[1:], dow_rolling_window.iloc[1:]
...
def get_market_open_duration(window):
    ...
    current_datetime = datetime.combine(previous_trading_day, current_time)
    market_start_time = datetime.combine(previous_trading_day, datetime.strptime("09:30:00", "%H:%M:%S").time())
    return (current_datetime - market_start_time).total_seconds() / 60  # in minutes
...
def calculate_insights(window, dow_window):
    if len(window) >= 5:
        ...
        ema = window['Close'].ewm(span=5, adjust=False).mean().iloc[-1]
        std = window['Close'].rolling(window=5).std().iloc[-1]
        bollinger_upper, bollinger_lower = rolling_avg + (2 * std), rolling_avg - (2 * std)
        ...
        # Calculate RS and RSI separately to avoid reference errors
        rs = avg_gain / avg_loss if avg_loss != 0 else float('nan')
        rsi = 100 - (100 / (1 + rs)) if not pd.isna(rs) else float('nan')
        dow_rolling_avg = dow_window['Close'].rolling(window=5).mean().iloc[-1]
        market_open_duration = get_market_open_duration(window)
        ...
        with metrics_section:
            ...
            col1.metric("5-min Rolling Avg", f"{rolling_avg:.2f}")
            col2.metric("EMA", f"{ema:.2f}")
            col3.metric("RSI", f"{rsi:.2f}")

            col4, col5, col6 = st.columns(3)
            col4.metric("Price Change", f"{price_change:.2f}")
            col5.metric("Volume Change", f"{volume_change}")
            col6.metric("Market Open Duration (min)", f"{market_open_duration:.2f}")
...
def get_natural_language_insights(
    ...
    with insight_section:
        insight_log.insert(0, f"{timestamp}: {response_text}")
        st.subheader("Natural Language Insights")
        st.write("\n\n".join(insight_log[:5]))  # Display the latest 5 insights
